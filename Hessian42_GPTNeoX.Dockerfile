...
# Run as: sudo DOCKER_BUILDKIT=1 docker build -t ineil77/hessian42-neox-exec:18072026 -f Hessian42_GPTNeoX.Dockerfile . 
...
# GPT-NeoX source (shallow clone; .git stripped)
...
# ---------------------------------------------------------------------------
# GPT-NeoX + Python training dependencies
#
# Kernel compilation and its build tree are confined to this single layer.
# ---------------------------------------------------------------------------
COPY --from=fetcher /out/gpt-neox /opt/gpt-neox
...
RUN --mount=type=cache,target=/root/.cache/pip \
 ...
 && pip install --no-cache-dir \
      ...
      setuptools==69.5.1 \
      six \
 && cp /tmp/Dockerfile_Scripts/install_neox_kernels.py /opt/gpt-neox/ \
 && python /opt/gpt-neox/install_neox_kernels.py \
 \
 # Strip build intermediates left by the fused-kernel compile.
 # The built .so files live alongside their sources and are kept;
 # object files, ninja logs, and setuptools scratch dirs are not.
 && find /opt/gpt-neox -type d -name build -prune -exec rm -rf {} + \
 && find /opt/gpt-neox -type d -name '*.egg-info' -prune -exec rm -rf {} + \
 && find /opt/gpt-neox -type d -name __pycache__ -prune -exec rm -rf {} + \
 && find /opt/gpt-neox \( -name '*.o' -o -name '*.obj' -o -name '*.cu.o' \
      -o -name '.ninja_deps' -o -name '.ninja_log' -o -name 'build.ninja' \) -delete \
 && rm -rf /root/.cache/torch_extensions/*/*/build \
 && rm -rf /tmp/pip-* /tmp/tmp* \
 && python -c "import deepspeed, transformers, tokenizers; print('neox deps import OK')"
