...
# Run as: sudo DOCKER_BUILDKIT=1 docker build -t ineil77/hessian42-cpu-exec:<tag> -f Hessian42_CPU.Dockerfile .
#
# CPU counterpart of Hessian42_GPU.Dockerfile: the same multi-language
# code-execution + rootless-firejail reward-sandbox stack on a plain Ubuntu 24.04
# base, minus the CUDA/torch stack and GPU/fabric tooling (no cuda base, torch,
# torch-tb-profiler, nvtop/nvitop, or InfiniBand/verbs packages). Determined
# compatibility (det-nobody user + libnss) is added the same way as the GPU image.
...
# per-exec jail via `-Mo --no_pivotroot` (mount(MS_MOVE)+chroot, avoiding the
# seccomp-blocked pivot_root); the reward path uses firejail, so this is a
# potential fallback backend. Build deps live only in this stage.
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,target=/var/lib/apt,sharing=locked \
    apt-get update -yqq \
 && DEBIAN_FRONTEND=noninteractive apt-get install -yqq --no-install-recommends \
...
# Determined compatibility: det-nobody user + libnss plugin (built from the
# bundled C sources with the make/gcc installed above), same as the GPU image.
